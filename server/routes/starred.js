...
router.post('/toggle', async (req, res) => {
  ...
  if (messageId) {
    ...
    sid = senderId;
    txt = text;
    const u = await dbGet('SELECT name, color FROM users WHERE id = ?', [senderId]);
    senderName = u?.name || 'Unknown';
    senderColor = u?.color || '#6366f1';
    const m = await dbGet('SELECT created_at FROM messages WHERE conversation_id = ? AND sender_id = ? AND text = ? ORDER BY created_at DESC LIMIT 1', [convId, sid, txt]);
    createdAt = m?.created_at || new Date().toISOString();
  } else {
    return res.status(400).json({ error: 'messageId (or conversationId + senderId + text) required' });
  }

  const isMember = await dbGet('SELECT 1 as ok FROM conversation_participants WHERE conversation_id = ? AND user_id = ?', [convId, req.userId]);
  if (!isMember) return res.status(403).json({ error: 'Not a member' });

  const existing = await dbGet('SELECT id FROM starred_messages WHERE user_id = ? AND conversation_id = ? AND sender_id = ? AND text = ?', [req.userId, convId, sid, txt]);
  if (existing) {
    await dbRun('DELETE FROM starred_messages WHERE id = ?', [existing.id]);
    return res.json({ starred: null });
  }

  const id = uuid();
  await dbRun('INSERT INTO starred_messages VALUES (?,?,?,?,?,?,?,?)', [id, req.userId, convId, sid, txt, senderName, senderColor, createdAt]);
