...
function ChatItem({ chat }) {
  ...
  const initials = chat.initials || getInitials(chat.name);
  const color = chat.color || getColor(chat.id);
  const lastMessage = chat.lastMessage ?? chat.last_message ?? '';
  const time = chat.time || formatTime(chat.last_message_time);
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between gap-2 mt-0.5">
            <span className="text-xs text-zinc-500 truncate">{lastMessage}</span>
            {chat.unread > 0 && (
